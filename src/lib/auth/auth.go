...
// NonceToken returns a nonce token for a request,
// and if necessary sets the cookie with our nonce
...
func NonceToken(writer http.ResponseWriter, request *http.Request) (string, error) {
	...
	// Get the secret from the session, or generate if none found
	nonce := session.Get(SessionNonceKey)
	if nonce == "" {
...
// CheckNonceToken checks the token against that
// stored in a session cookie, and returns an error if the check fails.
func CheckNonceToken(token string, request *http.Request) error {
	...
	// Get the secret from the session
	secret := session.Get(SessionNonceKey)
	if secret == "" {
		return fmt.Errorf("auth: error fetching nonce from session")
	}
