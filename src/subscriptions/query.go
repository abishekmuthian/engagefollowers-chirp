...
func NewWithColumns(cols map[string]interface{}) *Subscription {
	...
	subscription.AmountTotal = resource.ValidateFloat(cols["payment_gross"])
	subscription.AmountSubTotal = resource.ValidateFloat(cols["mc_gross"])
	...
	subscription.Invoice = resource.ValidateString(cols["invoice"])
...
// FindCustomerId fetches a single subscription record from the database by user id.
func FindCustomerId(userId int64) (*Subscription, error) {
	result, err := Query().Where("user_id=?", userId).FirstResult()
	if err != nil {
		return nil, err
	}
	return NewWithColumns(result), nil
